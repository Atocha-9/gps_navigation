...
class GPSPubClass():  
    ...
    def get_rotation (self, msg):
        ...
        self.robot_vel.Velocity = (kdist * dist_to_goal)/4
        self.robot_vel.Turn =  kang * dif_angle

        if (abs(dif_angle) > 0.2 and abs(dif_angle) < 0.25 ):
            self.robot_vel.Velocity = (self.robot_vel.Velocity - (self.robot_vel.Velocity / 4))/2
            self.robot_vel.Turn=  kang * dif_angle
            ...
            if(dist_to_goal  > (latlong/2.0) and dist_to_goal  < (latlong*500)):
                self.robot_vel.Velocity = (self.robot_vel.Velocity + 0.3)/2
                if (self.robot_vel.Velocity >= (kdist*dist_to_goal) ): 
                    self.robot_vel.Velocity = (kdist*dist_to_goal)/2
                if (self.robot_vel.Velocity >= vmax):
                    self.robot_vel.Velocity = vmax/2
            else:   
