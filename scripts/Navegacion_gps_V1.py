...
class GPSPubClass():  
    ...
    def get_rotation (self, msg):
        ...
        if (dif_angle < 6 and dif_angle > 0.2):
            self.robot_vel.linear.x = 0
            self.robot_vel.angular.z =  0.8 *-1*abs(dif_angle)
           
        elif (dif_angle < -0.2 and dif_angle > -6):
            self.robot_vel.linear.x = 0
            self.robot_vel.angular.z =  0.8 * abs(dif_angle)
            ...
            ## 1m se representa como 0.00001317690754 en terminos de  latitude y longitude 
            ## lo dividi entre 2 para quedarme a 50cm de distancia de separacion del goal  y con un rango de 1km    
            if(dist_to_goal  > 0.000006588 and dist_to_goal  < 0.01317):
                
                self.robot_vel.linear.x = 20000 * dist_to_goal
